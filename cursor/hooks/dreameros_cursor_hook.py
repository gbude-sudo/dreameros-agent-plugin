...
def _validate_part_descriptors(parts: Any, part_count: int, expected_chars: int) -> bool:
    """Validate ordered no-body continuation metadata without trusting a name."""
    ...
    for index, descriptor in enumerate(parts, start=1):
        if not _has_exact_keys(descriptor, {"part", "chars", "sha256"}):
            return False
        if descriptor.get("part") != index or not _valid_positive_int(descriptor.get("chars")):
            return False
        if not _is_lower_sha256(descriptor.get("sha256")):
            return False
        total += descriptor["chars"]
    return total == expected_chars
...
def _validate_session_package_result(payload: dict[str, Any]) -> tuple[bool, str]:
    ...
    if (
        ...
        or not _validate_part_descriptors(static_continuation.get("parts"), static_count, len(wrapper_body))
    ...
    if (
        ...
        or not _validate_part_descriptors(continuation.get("parts"), package_count, len(package_text))
